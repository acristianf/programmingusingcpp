...
struct Rectangle : Shape {
	...
	{
		if (h<=0 || w<=0) std::logic_error("Bad rectangle: non-positive side");
		add(xy);
	}
	Rectangle(Point x, Point y) :w{ y.x - x.x }, h{ y.y - x.y }
	{
		if (h<=0 || w<=0) std::logic_error("Bad rectangle: first point is not top left");
		add(x);
	}
...
struct Lines : Shape {	// indepentdent lines
	...
	Lines(std::initializer_list<Point> lst) 
		: Shape{lst} { 
			if (lst.size() % 2) std::logic_error("odd number of points for Lines"); 
			}
	void draw_lines() const;
